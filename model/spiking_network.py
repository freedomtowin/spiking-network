...
class SpikingNetwork:
    ...
    def lagValues(self,X,y,input_size,output_size,l1_regularization=0):
        ...
        self.lag_arr = np.zeros((num,X.shape[1]))
        ...
        for i in range(X.shape[1]):
            ...
            self.lag_arr[:,i] = t[peaks].flatten().astype(np.int)

        return self.lag_arr.astype(np.int)
    
    def autoregressiveComponents(self,X,y,input_size,output_size,l1_regularization=0):
        self.forward_list.append('ar')
        for i in range(self.lag_arr.shape[1]):
            #parameters for each lag value
            self._model_shapes.append((self.lag_arr.shape[0],output_size))
            self._number_of_parameters += self.lag_arr.shape[0]*output_size
